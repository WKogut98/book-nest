...
export const PATCH: RequestHandler = async ({request}) => {
    ...
    {
        return json({error: "No valid authorization header provided"}, {status: 401});
    }
    ...
    try
    {
        ...
        const {error: updateUsernameError} = await supabaseAdmin.from("user_names").update({name: userName}).eq("user_id", userId);
